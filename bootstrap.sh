...
if [[ "$REPLY" =~ ^[Yy]$ ]]; then
	...
	if [[ -f "$HOME"/.config/weechat/sec.conf ]]; then
		echo "A Weechat private configuration has been detected (sec.conf)."
...
echo "======= ALL DONE ======="
echo
echo "If anything has been modified, please reboot the computer for all the settings to be applied."
